...
$parentModule = 'PsApim'
if (-not $ModuleName) { $ModuleName = 'PsApim.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PsApim'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PsApim.psd1"
$prereqHash = @{
